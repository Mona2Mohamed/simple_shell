...
char **split_string(char *input_string, char *delimiter_string)
{
	...
	for (i = 0; input_string[i] != '\0'; i++)
		if (!is_delimiter(input_string[i], delimiter_string) && (is_delimiter(input_string[i + 1], delimiter_string) || !input_string[i + 1]))
			num_words++;
	...
	for (i = 0, j = 0; j < num_words; j++)
	{
		while (is_delimiter(input_string[i], delimiter_string))
			i++;
		k = 0;
		while (!is_delimiter(input_string[i + k], delimiter_string) && input_string[i + k])
			k++;
